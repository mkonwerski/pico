...
class i2c_slave:
    ...
    def read_is_pending(self):
        ...
        wróć wartość True, jeśli kontroler wydał polecenie I2C READ.

         Jeśli ta funkcja zwróci wartość True, oznacza to, że kontroler wydał komunikat
         I2C READ, co oznacza, że jego silnik I2C jest obecnie blokowany
         czekając, aż odpowiemy żądanymi danymi I2C READ.
        """
    ...
    def get(self):
        while not self.any():
            pass
        self.counter = mem32[self.i2c_base | self.IC_DATA_CMD] & 0xff
        return self.counter

    # ////////////////////////////
    def send(self, msg1):
        msg = list(bytearray(struct.pack('f', msg1)))

        for i in range(len(msg)):
            self.put_read_data(msg[i] & 0xff)
